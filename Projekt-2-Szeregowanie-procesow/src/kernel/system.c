...
FORWARD _PROTOTYPE( int do_setpri, (message *m_ptr) ); /*user def*/
FORWARD _PROTOTYPE( int do_setquant, (message *m_ptr) ); /*user def */
/*===========================================================================*
 *                              getproc                                      *
 *===========================================================================*/
 ...
 *				do_setpri                                    *
 *===========================================================================*/
 PUBLIC int do_setpri(message * m_ptr)
 {
   struct proc* rp;
   rp = getproc(m_ptr->m1_i1);
   if( rp == NIL_PROC )
   	return EINVAL;
   switch (m_ptr->m1_i2) {
   	case TYPE_A:
   		rp->proc_type = TYPE_A;
   		rp->time_left = quant[TYPE_A];
   		break;
   	case TYPE_B:
   		rp->proc_type = TYPE_B;
   		rp->time_left = quant[TYPE_B];
   		break;
   	case TYPE_C:
   		rp->proc_type = TYPE_C;
   		rp->time_left = quant[TYPE_C];
   		break;
   	}
  } 
/*===========================================================================*
 *                             do_setquant                                   *
 *===========================================================================*/
 PUBLIC int do_setquant(message *m_ptr)
 {
if(m_ptr->m1_i1<0 || m_ptr->m1_i2<0 || m_ptr->m1_i2>2)
   	return -1; 
   quant[m_ptr->m1_i2] = m_ptr->m1_i2;
   return (OK); 	
 }
 ...
 PUBLIC void sys_task()
 {
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETPRI:    r = do_setpri(&m);	break; /* user def */
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->proc_type = defGroup; /* user def */
  actualGroup = defGroup;
  defGroup++;
  if(defGroup == 3)
  	defGroup = 0;
  rpc->time_left = quant[defGroup];
  return(OK);
}
