 ...
 */
  ...
  for(Queue = actualGroup, t=0; t!=3; Queue= (Queue+1)%3, ++t)
  	if ( (rp = rdy_head[USER_Q+Queue]) != NIL_PROC) {
		proc_ptr = rp;
		bill_ptr = rp;
		actualGroup = rp->proc_type;
		return;
  	}
 ...
 */
   ...
   * bound processes.)
   */
  if (rdy_head[USER_Q+rp->proc_type] == NIL_PROC)
	rdy_tail[USER_Q+rp->proc_type] = rp;
  rp->p_nextready = rdy_head[USER_Q+rp->proc_type];
  rdy_head[USER_Q+rp->proc_type] = rp;
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	if ( (xp = rdy_head[USER_Q+rp->proc_type]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_Q+rp->proc_type] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
	...
	qtail = &rdy_tail[USER_Q+rp->proc_type];
  }
 ...
 */
  int Queue = actualGroup;
  int i;
  int t;
  for(i=(Queue+1)%3, t=0;t<3;i=(i+1)%3, ++t)
  ...
  rdy_head[USER_Q+Queue]->time_left = quant[Queue];
